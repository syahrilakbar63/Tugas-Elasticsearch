...
try:
    ...
    # Create index
    es.indices.create(index="mahasiswa", body=index_settings, ignore=[400])

    print("Index 'mahasiswa' berhasil dibuat.")
    ...
    with tqdm(total=total_data, desc="Importing Data", unit="doc") as pbar:
        for data in data_mongodb:
            ...
            es.index(index='mahasiswa', document=json.loads(json.dumps(data)))
            pbar.update(1)
